...
export default function SettingsCard({ icon, title, link, isActive = false }) {
  return (
    <Link href={link}>
      <div className="w-full h-[180px] p-6 rounded-xl my-border transition-all duration-300 cursor-pointer flex flex-col justify-between relative overflow-hidden settings-group card hover:shadow-lg">
        ...
        <div
          className={`relative text-4xl transition-all duration-300 p-3 rounded-full w-14 h-14 flex items-center justify-center z-10 ${
            isActive ? "active" : "bg-blue-100 text-blue-600"
          }  link`}
        >
          {icon}
        </div>
