...
const ArticleForm = ({ formData, setFormData, isUpdate, aiForm, setAiForm, mode, setMode }) => {
  ...
  const { loading, error, success, addArticle, editArticle, addArticleWithAI } =
  ...
  const handleSubmit = (e) => {
    ...
    isUpdate ? editArticle(form) : addArticle(form); // Use FormData instead of JSON
    setFormData({
      title: "",
      slug: "",
      content: "",
  ...
  const handleAiCategory = (id) => {
  setAiForm({ ...aiForm, category: id });
};

const handleAiSubCategory = (id) => {
  setAiForm({ ...aiForm, subcategory: id });
};

const handleAiTags = (e) => {
  const selectedTags = Array.from(
    e.target.selectedOptions,
    (option) => option.value
  );

  setAiForm({ ...aiForm, tag_id: selectedTags });
};
  const handleAIGenerate = () => {
  const payload = {
    title: aiForm.title,
    category: aiForm.category,
    subcategory: aiForm.subcategory,
    tag_id: aiForm.tag_id,
  };

  addArticleWithAI(payload);
};
  return (
    <form
        ...
        <>
          ...
          <div className="flex flex-col">
            ...
            <select
              ...
              onChange={(e) => handleAiCategory(e.target.value)}
         ...
         <div className="flex flex-col">
            ...
            <select
              name="subCategory"
              value={aiForm?.subcategory || ""}
              onChange={(e) => handleAiSubCategory(e.target.value)}
